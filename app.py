"""FastAPI web application for the paper-to-notebook tool (Azure Only)."""
...
from paper_to_notebook.config import MAX_PDF_SIZE_MB, DEFAULT_MODEL
from paper_to_notebook.core.pipeline import run_pipeline

# --- App setup ---
app = FastAPI(title="Paper to Notebook (Azure)", version="2.5", docs_url=None, redoc_url=None)
...
async def index():
    ...
    if not html_path.exists():
        return HTMLResponse(content="<h1>Static assets not found.</h1>", status_code=404)
...
async def generate(request: Request, file: UploadFile = File(...)):
    ...
    if size_mb > MAX_PDF_SIZE_MB:
        raise HTTPException(413, f"PDF too large ({size_mb:.1f}MB). Max is {MAX_PDF_SIZE_MB}MB.")
    ...
    async def event_stream():
        ...
        while not progress_queue.empty():
            ...
            _, step, name, detail, extra = event
            if extra and "draft_bytes" in extra: extra.pop("draft_bytes")
            data = {"step": step, "name": name, "detail": detail}
...
@app.get("/health")
async def health():
    return {"status": "ok", "package": "paper_to_notebook", "provider": "azure"}
